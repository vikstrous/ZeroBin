...
var util = {
    ...
    scriptLocation: function() {
        if(globalState.get('remote')) {
            ...
            return window.location.href.substring(0, window.location.href.length - window.location.search.length - window.location.hash.length - 1);
        }
    },
    /**
...
var Message = Backbone.Model.extend({
    defaults: {
        data: '',
        // in view mode: the base64 encoded and encrypted contents of the message; in preview mode: the plaintext
        meta: {
            language: 'none',
            postdate: 0
        }
...
var ReadPage = Backbone.View.extend({
    ...
    open_reply: function(e) {
        ...
        $('#replybutton').click(function(e) {
            ...
            $.post(util.scriptLocation(), data_to_send, 'json').error(function() {
    ...
    displayMessages: function() {
        ...
        $('#cleartext').snippet(paste.meta.language, {
            style: "ide-codewarrior"
        });
        // Display paste expiration.
        if(paste.meta.expire_date) {
            $('#remainingtime').html('<i class="icon-time"></i> This document will expire in ' + util.secondsToHuman(paste.meta.remaining_time) + '.').show();
        }
        if(paste.meta.burnafterreading) {
            $('#remainingtime').addClass('alert-error').html('<i class="icon-fire"></i> <strong>FOR YOUR EYES ONLY.</strong> Don\'t close this window, this message will self destruct.').show();
        }
    ...
    sendData: function() {
        ...
        if(files && files[0]) {
            ...
            reader.onload = _.bind(function(e) {
                ...
                this.uploadPaste(
                cipherdata, util.zeroCipher(randomkey, e.target.result), expiration, language, opendiscussion, randomkey);
            }, this);
            ...
            this.uploadPaste(
            cipherdata, undefined, expiration, language, opendiscussion, randomkey);
        }
...
var controller = {
    read: function(paste, key) {
        ...
        $.get(util.scriptLocation() + '?' + paste).error(function() {
